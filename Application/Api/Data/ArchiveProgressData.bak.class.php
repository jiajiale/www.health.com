...
class ArchiveProgressbakData extends BaseData{
    ...
    public function getUserInfo($conditions){
        ...
        $data = $this->table('__USERINFORMATION__ AS user')
            ...
            user.userMoney,user.userDiamond,user.userSex,user.backImageView AS backImage,achiveprogress.achivePoint')
            ->join('__ACHIVEPROGRESS__ as achiveprogress ON achiveprogress.userID = user.userID')
            ->where($where)
            ->find();
