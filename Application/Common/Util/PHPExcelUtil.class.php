...
class PHPExcelUtil {
    ...
    static public $config = array(
        ...
        'savepath'=>'/uploads/Admin/files/excel/',//上传子目录
    ...
    protected function UploadFile(){
        ...
        if(!empty($_FILES[$filename]['name'])){
            ...
            $path = $config['rootPath'].$config['savePath'];
    ...
    static public function export($data,$fields=null,$savename=null,$title='Sheet1',$suffix='xlsx'){
        ...
        foreach($data as $value){
            ...
            foreach($value as $key => $value2){
                ...
                $column_width[$j] = strlen($value2) > $column_width[$j] ? strlen($value2) : $column_width[$j];
                $j++;
            }
            $max = $max > $j ? $max : $j;
            $i++;
        ...
        // 设置单元格对其方式
        $objPHPExcel->getActiveSheet()->getStyle('A1:'.$letters_arr[$max-1].($i-1))->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_CENTER);
        $objPHPExcel->getActiveSheet()->getStyle('A1:'.$letters_arr[$max-1].($i-1))->getAlignment()->setVertical(PHPExcel_Style_Alignment::VERTICAL_CENTER);

        // 设置标题行颜色
        //$objPHPExcel->getActiveSheet()->getStyle('A1:E1')->getFill()->setFillType(PHPExcel_Style_Fill::FILL_SOLID);
        //$objPHPExcel->getActiveSheet()->getStyle('A1:E1')->getFill()->getStartColor()->setARGB("#0cedffb");
        $objPHPExcel->getActiveSheet()->getStyle('A1:'.$letters_arr[$max-1].'1')->getFont()->setBold(true);
        $objPHPExcel->getActiveSheet()->getStyle('A1:'.$letters_arr[$max-1].'1')->getFont()->setSize(14);

        // 设置单元格边框
        $objPHPExcel->getActiveSheet()->getStyle('A1:'.$letters_arr[$max-1].($i-1))->getBorders()->getAllBorders()->setBorderStyle(PHPExcel_Style_Border::BORDER_THIN);

        // 设置列宽
        foreach($column_width as $k => $v){
            $v = strlen($fields[$k]) > $v ? strlen($fields[$k]) : $v;
            $objPHPExcel->getActiveSheet()->getColumnDimension($letters_arr[$k])->setWidth(1.5*$v);
        }
